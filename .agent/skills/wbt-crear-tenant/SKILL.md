...
Una vez confirmado, ejecuta el script referenciando su ruta completa dentro de
esta skill (`.../wbt-crear-tenant/scripts/crear_tenant_workbeat.py`):
...
según `scripts/workbeat_config.json` (ver `scripts/workbeat_config.example.json`
...
ninguna otra carpeta. Solo falta que el usuario, en ese equipo, cree sus propios
`scripts/workbeat_config.json` y `scripts/workbeat_credentials.json` a partir de
las plantillas `.example.json`, con sus credenciales reales de ese entorno.
